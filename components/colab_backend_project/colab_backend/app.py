...
from colab_backend import __version__
from colab_backend.environment import Configuration, Environment
from colab_backend.router import router as root_router
...
def create_app(configuration: Configuration) -> FastAPI:
    ...
    app: FastAPI = FastAPI(
        ...
        root_path=f"{root_base}:{configuration.yw_port}/backends/colab_backend/{__version__}",
        lifespan=lifespan,
    )
